      ...
      <Card>
        <div className="p-4 sm:p-6 border-b border-border flex items-center justify-between flex-wrap gap-2">
          ...
          <label className="flex items-center gap-2 cursor-pointer text-sm text-muted-foreground">
            <Checkbox checked={includeCivilWar} onCheckedChange={(v) => setIncludeCivilWar(!!v)} />
            내전 경기 포함
          </label>
        </div>
              ...
              {sortedStats.length > 0 ? sortedStats.map((player, index) => (
                <tr key={player.id} className="hover:bg-muted/50 cursor-pointer" onClick={() => setSelectedPlayerForChart(player)}>
                  <td className="py-3 px-3 sm:py-4 sm:px-6">
                    <span data-testid={`text-rank-${index + 1}`} className={`text-xs font-bold px-2 py-1 rounded-full ${
                  ...
                  <td className="py-3 px-3 sm:py-4 sm:px-6">
                    <div className="flex items-center space-x-2 sm:space-x-3">
                      ...
                      <span data-testid={`text-player-name-${player.id}`} className="font-medium text-foreground text-sm sm:text-base">{player.name}</span>
                    </div>
                  </td>
                  <td data-testid={`text-appearances-${player.id}`} className={cellClass("appearances")}>{player.appearances}</td>
                  ...
                  <td data-testid={`text-assists-${player.id}`} className={cellClass("assists")}>{player.assists}</td>
                  <td data-testid={`text-saves-${player.id}`} className={`${cellClass("saves")} !text-emerald-600 font-semibold`}>{player.saves}</td>
                  <td data-testid={`text-attack-points-${player.id}`} className={cellClass("attackPoints")}>{attackPoints(player).toFixed(1)}</td>
                </tr>
              )) : (
          ...
          {chartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData} margin={{ top: 8, right: 16, left: -16, bottom: 0 }}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey={chartMeta?.key} stroke={chartMeta?.color} strokeWidth={2} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </DialogContent>
